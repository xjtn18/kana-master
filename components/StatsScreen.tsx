...
const StatsScreen: React.FC<StatsScreenProps> = ({
  ...
  const averageTime =
    totalAttemptedKana > 0 ? totalTimeMs / totalAttemptedKana : 0;
  ...
  const accuracy =
    totalAttemptedKana > 0 ? (perfectAnswers / totalAttemptedKana) * 100 : 0;
  ...
  if (isTimedSession && !didFinish) {
    ...
    if (accuracy < 80) {
      grade = "B";
      gradeColor = "text-blue-500 dark:text-blue-400";
    }
    if (accuracy < 60) {
      grade = "C";
      gradeColor = "text-yellow-500 dark:text-yellow-400";
    }
    if (accuracy < 40) {
      grade = "F";
  ...
  useEffect(() => {
    ...
    if (didFinish && (accuracy > 80 || isNewRecord)) {
      ...
      const colors = isNewRecord
        ? ["#F59E0B", "#FCD34D", "#FFFBEB"]
  ...
  return (
    <div className="w-full h-full overflow-y-auto overflow-x-hidden">
      <div className="max-w-4xl mx-auto p-6 pb-12">
        ...
        >
          <h2 className="text-3xl font-black text-slate-800 dark:text-slate-100 mb-2 transition-colors">
            Session Complete!
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col items-center justify-center col-span-1 md:col-span-1 transition-colors duration-300">
            <span className="text-sm font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">
              Rank
            </span>
            <span
              className={`font-black ${gradeColor} ${grade === "DNF" ? "text-4xl" : "text-6xl"}`}
            >
              {grade}
            </span>
          </div>
          ...
          <div className="md:col-span-3 grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col items-center justify-center transition-colors duration-300">
              {isTimedSession ? (
                <>
                  <ListChecks
                    className={`w-8 h-8 mb-3 ${didFinish ? "text-indigo-400 dark:text-indigo-300" : "text-slate-300 dark:text-slate-600"}`}
                  />
            ...
            {/* Avg Pace */}
            <div
              className={`bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border flex flex-col items-center justify-center transition-all duration-300 relative ${isNewRecord ? "border-amber-300 dark:border-amber-700 ring-2 ring-amber-100 dark:ring-amber-900/20 shadow-amber-50 dark:shadow-none" : "border-slate-100 dark:border-slate-700"}`}
            >
              <AnimatePresence>
                {isNewRecord && (
                  <motion.div
              ...
              <Clock
                className={`w-8 h-8 mb-3 transition-colors ${isNewRecord ? "text-amber-500 animate-bounce" : "text-indigo-400 dark:text-indigo-300"}`}
              />
              <span
                className={`text-2xl font-bold tabular-nums ${isNewRecord ? "text-amber-600 dark:text-amber-400" : "text-slate-800 dark:text-slate-100"}`}
              >
                {totalAttemptedKana > 0
                  ? currentPaceSeconds.toFixed(2)
                  : "0.00"}
                s
              </span>
              <span className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-lg border border-slate-100 dark:border-slate-700 overflow-hidden mb-12 transition-colors duration-300">
          <div className="p-6 bg-slate-50 dark:bg-slate-700/50 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
            <h3 className="font-bold text-slate-700 dark:text-slate-200 flex items-center">
              <AlertTriangle className="w-5 h-5 mr-2 text-rose-500 dark:text-rose-400" />{" "}
              Error Report
            </h3>
            ...
            {errorResults.length === 0 ? (
              ...
              <div className="w-full grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {errorResults.map((res, idx) => (
                  <div
                    key={idx}
                    className="relative p-3 rounded-xl border-2 border-rose-100 dark:border-rose-900/40 bg-rose-50 dark:bg-rose-900/20 flex flex-col items-center"
                  >
                    <span
                      className={`text-3xl mb-1 text-slate-800 dark:text-slate-100 ${fontClass}`}
                    >
                      {res.char.char}
                    </span>
